...
for feature_idx in tqdm(top_feature_indices):
    feature_idx = feature_idx.item()
    print(f"\n--- Feature Index: {feature_idx} ---")

    max_activation = -1.0  # 初始化最大激活值为一个较小的数
    max_activation_sample_idx = -1
    max_activation_pos_idx = -1
    ...
    for chunk_idx in range(4):
        ...
        # 找到当前块中的最大激活值和索引
        chunk_max_activation = feature_activations_chunk.max()
        if chunk_max_activation > max_activation:
            max_activation = chunk_max_activation
            chunk_max_activation_indices = np.where(feature_activations_chunk == max_activation)
            max_activation_sample_idx_in_chunk, max_activation_pos_idx = chunk_max_activation_indices[0][0], chunk_max_activation_indices[1][0]

            # 将块内的样本索引转换为整个数据集中的样本索引
            max_activation_sample_idx = chunk_idx * 500 + max_activation_sample_idx_in_chunk
    ...
    input_tokens = dataset[int(max_activation_sample_idx)]["input_ids"]

    # 打印上下文
    context_window = 10
    start_idx = max(0, max_activation_pos_idx - context_window)
    end_idx = min(len(input_tokens), max_activation_pos_idx + context_window + 1)
    context_tokens = input_tokens[start_idx:end_idx]

    decoded_text = tokenizer.decode(context_tokens)
    decode_max_idx = tokenizer.decode(input_tokens[max_activation_pos_idx:max_activation_pos_idx+1])
    print("---------------------------")
    print(f"Max Activation Value: \n{(start_idx,max_activation_pos_idx,end_idx)}\n{decode_max_idx}\n{max_activation}")
